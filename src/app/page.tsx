...
import { Terminal, Bot, Sparkles, Github, Code2 } from "lucide-react";
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-foreground selection:bg-purple-500/30">
      ...
      {/* Prerequisites */}
      <section id="prerequisites" className="py-24 bg-muted/30 dark:bg-background relative border-y border-border/50 dark:border-white/10">
        <div className="container mx-auto px-4 max-w-5xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold tracking-tight sm:text-4xl text-foreground dark:text-white">{t("prerequisites.title")}</h2>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {[
              { id: 1, icon: Terminal, color: "text-blue-500 dark:text-blue-400", shadow: "dark:drop-shadow-[0_0_8px_rgba(96,165,250,0.5)]" },
              { id: 2, icon: Code2, color: "text-purple-500 dark:text-purple-400", shadow: "dark:drop-shadow-[0_0_8px_rgba(192,132,252,0.5)]" },
              { id: 3, icon: Bot, color: "text-green-500 dark:text-green-400", shadow: "dark:drop-shadow-[0_0_8px_rgba(74,222,128,0.5)]" },
              { id: 4, icon: Sparkles, color: "text-yellow-500 dark:text-yellow-400", shadow: "dark:drop-shadow-[0_0_8px_rgba(250,204,21,0.5)]" }
            ].map((item) => {
              const Icon = item.icon;
              return (
                <div key={item.id} className="flex flex-col items-start gap-4 p-8 rounded-2xl bg-card dark:bg-white/5 border border-border/50 dark:border-white/10 transition-all hover:bg-accent/50 dark:hover:bg-white/10">
                  <div className="p-3 bg-muted/50 dark:bg-white/5 rounded-xl border border-border/50 dark:border-white/10 mb-2">
                    <Icon className={`h-8 w-8 ${item.color} ${item.shadow}`} />
                  </div>
                  <span className="text-[1.1rem] text-muted-foreground dark:text-gray-300 text-start leading-relaxed">{t(`prerequisites.item${item.id}`)}</span>
                </div>
              );
            })}
          </div>
      ...
      <section id="contact" className="py-24 bg-background relative overflow-hidden">
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=2670&auto=format&fit=crop')] bg-cover bg-center opacity-5 dark:mix-blend-screen grayscale" />
        ...
        <div className="container relative mx-auto px-4 max-w-4xl text-center z-20">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-center gap-6 lg:gap-10">
             <a href="tel:05X-XXXXXXX" className="phone-link text-4xl font-black text-foreground dark:text-white hover:text-muted-foreground dark:hover:text-gray-200 transition-colors" dir="ltr">
              05X-XXXXXXX
             </a>
